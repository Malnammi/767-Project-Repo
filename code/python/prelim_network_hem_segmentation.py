...
@author: Moeman
Purpose: perliminary network for training on hemorrages segmentation 
...
y_train = np.array([np.array(Image.open(fname)) for fname in glob.glob(diaret_dir+"/train/hem_512/*")])
X_test = np.array([np.array(Image.open(fname)) for fname in glob.glob(diaret_dir+"/test/images_512_clahe/*")])
y_test = np.array([np.array(Image.open(fname)) for fname in glob.glob(diaret_dir+"/test/hem_512/*")])
