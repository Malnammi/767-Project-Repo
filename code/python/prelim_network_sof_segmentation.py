...
@author: Moeman
Purpose: perliminary network for training on soft exudates segmentation 
...
y_train = np.array([np.array(Image.open(fname)) for fname in glob.glob(diaret_dir+"/train/sof_512/*")])
X_test = np.array([np.array(Image.open(fname)) for fname in glob.glob(diaret_dir+"/test/images_512_clahe/*")])
y_test = np.array([np.array(Image.open(fname)) for fname in glob.glob(diaret_dir+"/test/sof_512/*")])
