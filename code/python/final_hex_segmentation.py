...
#load training sets
X_train = np.array([np.array(Image.open(fname.replace('hex_128\\Hard_exudates_', 'images_128_clahe\\image_'))) 
                                            for fname in glob.glob(diaret_dir+"/train/hex_128/*")])
y_train = np.array([np.array(Image.open(fname)) for fname in glob.glob(diaret_dir+"/train/hex_128/*")])
X_test = np.array([np.array(Image.open(fname.replace('hex_128\\Hard_exudates_', 'images_128_clahe\\image_'))) 
                                        for fname in glob.glob(diaret_dir+"/test/hex_128/*")])
y_test = np.array([np.array(Image.open(fname)) for fname in glob.glob(diaret_dir+"/test/hex_128/*")])
...
model_cnn.compile(loss=dice_coef_loss, optimizer='adam', 
              metrics=[dice_coef])
...
np_dice_coef(y_test, model_cnn.predict(X_test) > 0.5)